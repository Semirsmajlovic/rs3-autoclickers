...
def main():
    ...
    logger.info(f"📏 DPI Scaling: {DPI_SCALE:.2f}x ({DPI_SCALE*100:.0f}%)")
    logger.info(f"🖥️  Resolution: 3840x2160 (4K)")
    logger.info(f"📐 Base Coordinates (1920x1080):")
    logger.info(f"   🕳️  Dung Hole: {BASE_DUNG_HOLE_REGION}")
    logger.info(f"   🍋 Lemon Sour: {BASE_LEMON_SOUR_REGION}")
    if USE_HOLE_IN_ONE:
        logger.info(f"   🏌️  Hole in One: {BASE_HOLE_IN_ONE_REGION}")
    logger.info(f"📐 Scaled Coordinates ({3840}x{2160}):")
    logger.info(f"   🕳️  Dung Hole: {DUNG_HOLE_REGION}")
    logger.info(f"   🍋 Lemon Sour: {LEMON_SOUR_REGION}")
    if USE_HOLE_IN_ONE:
        logger.info(f"   🏌️  Hole in One: {HOLE_IN_ONE_REGION}")
    logger.info("─" * 60)
